...
{
	z->w[0] = _rv32_rev_b(z->w[0]);
	z->w[1] = _rv32_rev_b(z->w[1]);
	z->w[2] = _rv32_rev_b(z->w[2]);
	z->w[3] = _rv32_rev_b(z->w[3]);
}
...
void ghash_mul_rv32(gf128_t * z, const gf128_t * x, const gf128_t * h)
{
	...
	//	4 x GREV
	x0 = _rv32_rev_b(x0);					//	reverse input x only
	x1 = _rv32_rev_b(x1);
	x2 = _rv32_rev_b(x2);
	x3 = _rv32_rev_b(x3);
	...
	z4 = _rv32_clmulh(x3, y);
	z3 = _rv32_clmul(x3, y);
	t1 = _rv32_clmulh(x2, y);
	z2 = _rv32_clmul(x2, y);
	z3 = z3 ^ t1;
	t1 = _rv32_clmulh(x1, y);
	z1 = _rv32_clmul(x1, y);
	z2 = z2 ^ t1;
	t1 = _rv32_clmulh(x0, y);
	z0 = _rv32_clmul(x0, y);
	z1 = z1 ^ t1;

#ifdef NO_SHIFTRED
	//	Mul reduction: 1 x CLMULH, 1 x CLMUL, 2 x XOR
	t1 = _rv32_clmulh(z4, 0x87);
	t0 = _rv32_clmul(z4, 0x87);
	z1 = z1 ^ t1;
	z0 = z0 ^ t0;
#else
...
void ghash_mul_rv32_kar(gf128_t * z, const gf128_t * x, const gf128_t * h)
{
	...
	z7 = _rv32_clmulh(x3, y3);				//	high pair
	z6 = _rv32_clmul(x3, y3);
	z5 = _rv32_clmulh(x2, y2);
	z4 = _rv32_clmul(x2, y2);
	t0 = x2 ^ x3;
	t2 = y2 ^ y3;
	t1 = _rv32_clmulh(t0, t2);
	t0 = _rv32_clmul(t0, t2);
	t1 = t1 ^ z5 ^ z7;
	t0 = t0 ^ z4 ^ z6;
	z6 = z6 ^ t1;
	z5 = z5 ^ t0;

	z3 = _rv32_clmulh(x1, y1);				//	low pair
	z2 = _rv32_clmul(x1, y1);
	z1 = _rv32_clmulh(x0, y0);
	z0 = _rv32_clmul(x0, y0);
	t0 = x0 ^ x1;
	t2 = y0 ^ y1;
	t1 = _rv32_clmulh(t0, t2);
	t0 = _rv32_clmul(t0, t2);
	...
	x3 = _rv32_clmulh(t1, t3);				//	middle
	x2 = _rv32_clmul(t1, t3);
	x1 = _rv32_clmulh(t0, t2);
	x0 = _rv32_clmul(t0, t2);
	...
	t1 = _rv32_clmulh(z7, 0x87);
	t0 = _rv32_clmul(z7, 0x87);
	z4 = z4 ^ t1;
	z3 = z3 ^ t0;
	t1 = _rv32_clmulh(z6, 0x87);
	t0 = _rv32_clmul(z6, 0x87);
	z3 = z3 ^ t1;
	z2 = z2 ^ t0;
	t1 = _rv32_clmulh(z5, 0x87);
	t0 = _rv32_clmul(z5, 0x87);
	z2 = z2 ^ t1;
	z1 = z1 ^ t0;
	t1 = _rv32_clmulh(z4, 0x87);
	t0 = _rv32_clmul(z4, 0x87);
