...
//	XXX	need to find optimization for these
...
#define SBOX64_ENC 0x21748FE3DA09B65CLL
#define SBOX64_DEC 0xA970364BD21C8FE5LL
...
//	one way to decompose a 4x4 permutation into two for rv32

#define RV32_XPERM4_P64(p, x)                                                  \
  (_rv32_xperm4((p)&0xFFFFFFFF, (x)) |                                         \
   _rv32_xperm4((p) >> 32, (x) ^ 0x88888888))

//	testing

static inline uint64_t dec_s_layer(uint64_t x) {
  uint32_t w0 = (uint32_t)x;
  uint32_t w1 = (uint32_t)(x >> 32);

  w0 = RV32_XPERM4_P64(SBOX64_DEC, w0);
  w1 = RV32_XPERM4_P64(SBOX64_DEC, w1);

  return ((uint64_t)w0) | (((uint64_t)w1) << 32);
}

static inline uint64_t enc_s_layer(uint64_t x) {
  uint32_t w0 = (uint32_t)x;
  uint32_t w1 = (uint32_t)(x >> 32);

  w0 = RV32_XPERM4_P64(SBOX64_ENC, w0);
  w1 = RV32_XPERM4_P64(SBOX64_ENC, w1);
...
void present80_key(uint64_t rk[32], const uint8_t key[10]) {
  ...
  for (i = 1; i < 32; i++) {
    ...
    k1 = (t & ~(0xFllu << 60)) | (((SBOX64_ENC) >> ((t >> 58) & 0x3C)) << 60);
...
void present128_key(uint64_t rk[32], const uint8_t key[16]) {
  ...
  for (i = 1; i < 32; i++) {
    ...
    k1 = (t & ~(0xFFllu << 56)) | (((SBOX64_ENC) >> ((t >> 58) & 0x3C)) << 60) |
